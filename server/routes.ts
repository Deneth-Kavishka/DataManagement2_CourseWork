...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/vendors", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch vendors", error: String(error) });
    }
  });
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    try {
      ...
      await storage.updateUser(user.id, { resetToken: resetToken });
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      const { password: _, verificationToken, resetToken, ...userWithoutPassword } = user;
